...
protected:
  ...
  static Handle<Value> Bop(Nan::NAN_METHOD_ARGS_TYPE info, int op);
};
...
NAN_METHOD(BigNum::New)
{
  if (!info.IsConstructCall()) {
    ...
    Handle<Value> newInst = Nan::New<FunctionTemplate>(constructor_template)->
...
NAN_METHOD(BigNum::ToString)
{
  ...
  Handle<Value> result = Nan::New<String>(to).ToLocalChecked();
  OPENSSL_free(to);
...
NAN_METHOD(BigNum::BitLength)
{
  ...
  Handle<Value> result = Nan::New<Integer>(size);
